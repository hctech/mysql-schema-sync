...
class SyncSchemaStep(object):
    def __init__(self, fromdb, todb, step):
        """
    ...
    @property
    def msg(self):
        return "** sync mysql schema from database `%s-%s` to database `%s-%s`, current step is %s **" % (
            self.fromdb.parent.host,
            self.fromdb.name,
            self.todb.parent.host,
            self.todb.name,
            self.step
        )
...
def sync_schema(fromdb, todb, options):
    ...
    yield SyncSchemaStep(fromdb, todb, 'sync database options'), ''
    if p and r:
        yield (
            "%s %s;" % (todb.alter(), p),
            "%s %s;" % (todb.alter(), r)
    ...
    yield SyncSchemaStep(fromdb, todb, 'sync tables, create table'), ''
    ...
    yield SyncSchemaStep(fromdb, todb, 'sync tables, drop table'), ''
    for p, r in sync_dropped_tables(fromdb.tables, todb.tables,
                                    sync_auto_inc=options['sync_auto_inc'],
                                    sync_comments=options['sync_comments']):
        yield p, r

    yield SyncSchemaStep(fromdb, todb, 'sync cloumns, index, foreignkey, table options'), ''
